...
def _oracle_quant(x):
    ...
    dequant = (lut[(code & 7).long()] * torch.where((code & 8) != 0, -1, 1) * scale[..., None]).to(torch.bfloat16)
...
def _allocate(q, keys, lengths, *, max_candidates=0, source=False, high_pages=False,
              max_rows=None, page_order=None):
    device = q.device
    ...
    base = 2**31 // api.MXFP4_INDEX_PAGE_BYTES + 17 if high_pages else 7
    physical = page_order + base
    pool = torch.empty((base + pages, api.MXFP4_INDEX_PAGE_BYTES), device=device, dtype=torch.uint8)
    slots = physical[torch.arange(width, device=device) // 64].long() * 64 + torch.arange(width, device=device) % 64
    ...
    plan = api.plan(api.Caps(
        device=device, num_q_heads=heads, max_q_rows=max_rows or rows,
        max_page_table_width=pages, topk=512, cache_format="mxfp4",
        max_candidates=max_candidates, candidate_topk_blocks=2048 if source else 0,
    ))
    spec, = plan.scratch_specs()
    args = dict(
        scratch=torch.empty(spec.shape, device=device, dtype=spec.dtype),
        q_mxfp4=packed, q_scales=sf, index_k_cache=pool, page_table=physical[None],
        cache_lengths=lengths, active_width=torch.tensor([width], dtype=torch.int32, device=device),
...
def _assert_topk(scores, output, output_scores, logical_positions=None):
    for row in range(scores.shape[0]):
        ...
        if logical_positions is None:
            torch.testing.assert_close(output_scores[row, :count], scores[row, selected].float(), rtol=0, atol=0)
        else:
            assert bool(torch.isin(selected, logical_positions[row, valid].long()).all())
...
def test_per_group_quantization_and_permuted_high_page_writer():
    ...
    q = (q.view(2, 4, 4, 32) * torch.tensor([2.0**-120, .03125, 8, 256], device=device)[None, None, :, None]).to(torch.bfloat16).view_as(q)
    q[:, 0, :32] = 0
    # Exact E2M1 half-way boundaries, including signs.
    q[0, 1, :8] = torch.tensor([.25, .75, 1.25, 1.75, 2.5, 3.5, 5, 6], device=device)
    keys = torch.randn((130, 128), device=device, dtype=torch.bfloat16)
    lengths = torch.tensor([0, 129], dtype=torch.int32, device=device)
    plan, args = _allocate(q, keys, lengths, high_pages=True)
    ...
    positions = torch.arange(130, device=device)
    pages = args["page_table"][0, positions // 64].long()
    data_offsets = (positions % 64)[:, None] * 64 + torch.arange(64, device=device)
    scale_offsets = 4096 + (positions % 64)[:, None] * 4 + torch.arange(4, device=device)
    ...
    torch.testing.assert_close(scores[:, :130], expected, rtol=0, atol=0)
    assert bool(torch.isneginf(scores[:, 130:]).all())
    api.select(binding)
...
def test_source_blockmax_newest_and_bounded_candidate_reindex():
    ...
    binding = api.bind(plan, query_weights=torch.ones((3, 4), dtype=torch.bfloat16, device=device), **args)
    scores = api.score(binding)
    ...
    torch.testing.assert_close(candidates[1, :9], torch.arange(9, dtype=torch.int32, device=device))
    torch.testing.assert_close(candidates[2, :2047 * 8], torch.arange(2047 * 8, dtype=torch.int32, device=device))
...
def test_fixed_graph_buffers_multiple_live_rows_and_visibility():
    ...
    plan, args = _allocate(q, keys, lengths, max_candidates=128)
    candidates = torch.arange(128, device=device, dtype=torch.int32).expand(4, -1).contiguous()
    ...
    try:
        for rows in (1, 3, 4):
            api.quantize_q_mxfp4(
                q[:rows], q_mxfp4=args["q_mxfp4"][:rows],
                q_scales=args["q_scales"][:rows],
            )
            live = dict(args)
            for name in ("q_mxfp4", "q_scales", "cache_lengths", "output_indices", "output_scores", "candidate_indices", "candidate_lengths"):
        ...
        for visible in (0, 5, 101):
            ...
            assert [view.data_ptr() for view in binding.runtime.scratch.values()] == addresses
            expected.fill_(-1)
