...
def _page_table_width_for_scratch(caps: "SPARKINFERPagedAttentionScratchCaps") -> int:
...
class SPARKINFERPagedAttentionScratchCaps:
...
class _SPARKINFERPagedAttentionScratchLayout:
    nbytes: int
...
def _paged_attention_scratch_layout(
...
) -> _SPARKINFERPagedAttentionScratchLayout:
    offset = 0
    ...
    return _SPARKINFERPagedAttentionScratchLayout(
...
def _make_plan_metadata_cache(
    ...
    *,
    device: torch.device,
) -> _SPARKINFERPagedPlanMetadataCache:
    return _SPARKINFERPagedPlanMetadataCache(
...
class SPARKINFERPagedAttentionScratch:
    ...
    _plan_metadata_cache: _SPARKINFERPagedPlanMetadataCache | None = None
    ...
    def _copy_cached_plan_metadata(
        self,
        cache: _SPARKINFERPagedPlanMetadataCache,
    ) -> None:
...
@dataclass(frozen=True, kw_only=True)
class SPARKINFERPagedAttentionBinding:
    scratch: object
    q: torch.Tensor
    k_cache: torch.Tensor
    v_cache: torch.Tensor
...
def build_paged_attention_binding(
    *,
    ...
    return SPARKINFERPagedAttentionBinding(
        scratch=scratch,
        q=q,
        k_cache=k_cache,
        v_cache=v_cache,
...
def _materialize_paged_attention_scratch(
    ...
    *,
    ...
    plan_metadata_cache: _SPARKINFERPagedPlanMetadataCache | None,
...
def plan_paged_attention_scratch(
    ...
    return SPARKINFERPagedAttentionScratchPlan(
        caps=caps,
        layout=layout,
        _scratch_specs=(
            scratch_buffer_spec(
