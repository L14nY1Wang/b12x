...
from cutlass.cutlass_dsl import (
    Int32, Uint8, Uint32, Uint64, Float32, T, dsl_user_op,
)
...
def sm120_make_smem_layout_sfa(
    ...
    *, loc=None, ip=None,
    ...
    sM = (mn_bs, tile_shape_mnk[0] // blk_mn)
    stM = (mn_st, blk_elems)
    sK = (
        k_bs,
        blk_sf // mma_nsf,
    ...
    return cute.append(
        sl,
        cute.make_layout(
            num_stages, stride=cute.cosize(cute.filter_zeros(sl)),
        ),
    )
...
class _Pointer(Pointer):
    def __init__(
        self, pointer, dtype,
        mem_space=_cute_ir.AddressSpace.generic,
        assumed_align=None,
    ):
        ...
        self._assumed_align = (
            dtype.width // 8 if assumed_align is None else assumed_align
        )
        self._desc = None
    ...
    def align(self, min_align, *, loc=None, ip=None):
...
def make_ptr(dtype, value, mem_space=AddressSpace.generic, assumed_align=None):
    addr = (
        ...
        else ctypes.cast(value, ctypes.c_void_p).value
    )
    return _Pointer(addr, dtype, mem_space, assumed_align=assumed_align)
    ...
    def __call__(
        ...
        fp4_atom = cute.nvgpu.warpgroup.make_smem_layout_atom(
            sm90_utils.get_smem_layout_atom(
                utils.LayoutEnum.COL_MAJOR, ab, 128,
            ),
            ab,
        )
        fp4_staged = cute.tile_to_shape(
            fp4_atom, (128, 128, 1), order=(0, 1, 2),
        )

        bf16_atom = cute.nvgpu.warpgroup.make_smem_layout_atom(
            sm90_utils.get_smem_layout_atom(
                utils.LayoutEnum.COL_MAJOR, bf, 128,
            ),
            bf,
        )
        ...
        perm = sm120_utils.get_permutation_mnk(
            self.tile_shape_mnk, 16, False,
        )
        ...
        sfa_staged = sm120_make_smem_layout_sfa(
            tiled_mma, self.tile_shape_mnk, 16, 1,
        )
    ...
    def kernel(
        ...
        class S:
            ...
            sSFA: cute.struct.Align[
                cute.struct.MemRange[cutlass.Float8E4M3FN, sfa_cs], 1024
            ]

        st = smem.allocate(S)
        sBF16 = st.sBF16.get_tensor(
            bf16_smem.outer, swizzle=bf16_smem.inner,
        )
        sFP4 = st.sFP4.get_tensor(
            fp4_smem.outer, swizzle=fp4_smem.inner,
        )
            ...
            while tile_idx < total_tiles:
                ...
                while blk < Int32(128 * 8):
                    ...
                    for e in cutlass.range_constexpr(16):
                        v = cutlass.Float32(
                            sBF16[row, col0 + Int32(e), stage],
                        )
                        vals[e] = v
                        bmax = fmax_f32(bmax, fabs_f32(v))
                    p64, sbyte = quantize_block_fp4_fast(
                        vals, bmax, gs_value,
                    )
        ...
        # DMA warp 4: TMA loads only.
            ...
            cute.arch.setmaxregister_decrease(
                self.load_register_requirement,
            )
